...
class Agent():
  ...
  def learn(self, mem):
    # Sample transitions
    states, actions, returns, next_states, nonterminals = mem.sample_uniform(self.batch_size)
    #  idxs, states, actions, returns, next_states, nonterminals, weights = mem.sample(self.batch_size)

    # Calculate current state probabilities (online network noise already sampled)
    ps = self.online_net(states, log=True)  # Log probabilities log p(s_t, ·; θonline)
    ps_a = ps[range(self.batch_size), actions]  # log p(s_t, a_t; θonline)
    ...
    with torch.no_grad():
      ...
      expect = returns + nonterminals.squeeze() * (self.discount ** self.n) * pns_a  # Tz = R^n + (γ^n)z (accounting for terminal states)
